...
namespace GraphVisualizer.Services
{
    ...
    public class ScriptLoaderService
    {
        ...
        ConcurrentDictionary<string, MetadataReference> MetadataReferenceCache = new ConcurrentDictionary<string, MetadataReference>();
        internal async Task<MetadataReference> GetAssemblyMetadataReference(Assembly assembly)
        {
        ...
        internal async Task<MetadataReference> GetAssemblyMetadataReference(string assemblyName)
        {
            MetadataReference? ret = null;
            if (MetadataReferenceCache.TryGetValue(assemblyName, out ret)) return ret;
            var assemblyUrl = $"./_framework/{assemblyName}.dll";
            ...
            {
                Console.WriteLine($"metadataReference not loaded: {assemblyName} {ex.Message}");
            }
            if (ret == null) throw new Exception("ReferenceMetadata nto found. If using .Net 8, <WasmEnableWebcil>false</WasmEnableWebcil> must be set in the project .csproj file.");
            MetadataReferenceCache[assemblyName] = ret;
            return ret;
